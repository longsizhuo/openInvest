...
跑法：
  uv run python -m scripts.sync_gui_dist               # 拉最新（dist-latest tag，每次 push main 自动更新）
  uv run python -m scripts.sync_gui_dist --tag <tag>   # 指定 tag（pinned 版本）
  uv run python -m scripts.sync_gui_dist --owner X --repo Y --tag T  # fork 用户改这里
