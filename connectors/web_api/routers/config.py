...
from connectors.web_api.models import ConfigItem, ConfigResponse, ConfigUpdateRequest
...
def _view() -> ConfigResponse:
    from core.config import effective_api_config
...
async def put_config(body: ConfigUpdateRequest = Body(...)) -> ConfigResponse:
    ...
    from core.config import set_persisted_override
    try:
...
async def delete_config(key: str) -> ConfigResponse:
    ...
    from core.config import clear_persisted_override
    try:
