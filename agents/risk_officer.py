...
**你有工具可调用**：
- `query_dreaming_insights(asset_symbol="{asset['symbol']}", top_k=3)` → 长期行为模式（用户过去类似情境的过度集中持仓 / 情绪化追涨等）
- `get_recent_committee_verdicts(asset_symbol="{asset['symbol']}", n=5)` → 上次同资产委员会决策，看决策一致性
