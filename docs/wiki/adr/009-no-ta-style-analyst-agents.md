...
- ~~单边牛市窗口结论；熊市/震荡窗未覆盖~~ → **2026-06-12 复测已覆盖**（wiki 16
  §5b）：2022 熊市窗口 × 3 模型家族 + ensemble 三种组合。fundamental 在
  2022×flash 单格过 Gate，但跨模型 1/3（reasoner/ds-chat 均收敛到机械映射
  水平）——按裁决前预注册的"2/3 才算信号"标准，孤证不立，**维持原判且证据更强**
- 推翻本 ADR 的条件（复测后**收紧**）：某分析师过预注册 Gate **且跨 ≥2 模型
  家族复现** → 跑 Phase B 集成 A/B（代码已在 test_ta 分支备好，flag 默认关）。
...
- 新闻维度仍留前向实验队列（事件层攒数据；GDELT 2022 回测补测进行中）
