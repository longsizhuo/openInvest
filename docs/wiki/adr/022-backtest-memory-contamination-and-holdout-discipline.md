...
### 更新 2026-07-22:委员会模型 MiMo → DeepSeek,cutoff 常量不变

- 委员会/回测执行模型已切换为 DeepSeek(deepseek-chat,V3 系)。其公开口径的训练数据
  截止(~2024 年中)不晚于 MiMo 自报的 2024 末,`LLM_TRAINING_CUTOFF = "2024-12-31"`
  **保持不变**——对 DeepSeek 这个边界只会更保守,分桶纪律原样适用。下文以 MiMo 为例的
  实证段落保留作历史证据,机理对任何 LLM 同样成立。
- **换模型/升级版本时必须重验 cutoff**(实测模型自报 + 查发布说明,取更晚者)并同步
  上调常量——干净桶是**相对模型**的:升级到含 2025 语料的版本时,原 holdout 整桶
  重新变脏。
