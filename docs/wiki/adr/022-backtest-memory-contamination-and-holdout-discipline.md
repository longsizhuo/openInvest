...
### 更新 2026-07-22:实测运行时模型为 deepseek-v4-flash,cutoff 上调至 2025-05-31

- 运行时模型**实为 `deepseek-v4-flash`**(billing 面板 + `get_llm_config()` 双确认)——
  不是切换时假设的 V3 系。按本 ADR 验法实测:自报训练数据截止 **2025 年 5 月**;
  2026 年事件探针答"不知道"。`CONTAMINATION_CUTOFF` 由 2024-12-31 **上调至
  2025-05-31**(取自报月末,保守向后)。2025-01→05 段对 v4-flash 降级为一致性桶。
- 上调后的实测对照(backtest 干净桶,2026-07-22):真干净段(>2025-05-31,n=1099)
  30d 命中 68.4%(剔宏观突变 72.7%)vs 降级段(2025-01→05,n=546)68.7%(78.2%)——
  记忆通胀量级 ~0-5.5pp,基线在真干净段站得住。
- **教训:cutoff 是相对模型的,而模型会在配置默认值层面悄悄换代**。每次跑 holdout
  前先 `get_llm_config()` 确认 model 并重验自报 cutoff(实测自报 + 查发布说明,取
  更晚者)。下文 MiMo 实证段保留作历史证据;review 账本逐行补 provider 字段以支持
  跨模型混合账本的逐行分桶,列为后续。
- 唯一不受模型升级影响的干净数据源是**前瞻纸面运行**(决策时未来尚不存在,任何未来
  模型都不可能记得),见 `experiments/paper_fleet/`。
