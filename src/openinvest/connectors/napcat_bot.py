...
from openinvest.utils.gold_price import format_gold_report, get_gold_snapshot, infer_offset_pct
...
def _payday(ctx: CommandContext) -> str:
    from openinvest.jobs.payday_check import run as payday_run
    result = payday_run()
...
def _run(ctx: CommandContext) -> str:
    ...
    def _bg():
        try:
            from openinvest.jobs.daily_report import run as dr_run
            result = dr_run()
