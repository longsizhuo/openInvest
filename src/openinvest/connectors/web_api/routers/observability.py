...
async def get_jobs_status() -> JobsStatusResponse:
    ...
    jobs_dir = Path(_jobs_pkg.__file__).parent
...
async def get_llm_usage(
    ...
    from openinvest.core.llm_telemetry import read_telemetry
...
async def get_llm_summary(
    ...
    from openinvest.core.llm_telemetry import telemetry_summary
...
async def get_data_sources_health(pm: PortfolioManager = Depends(get_pm)) -> DataSourcesHealthResponse:
    ...
    # 10: market DB
    db_path = INVEST_ROOT / "db" / "market_data.db"
    if db_path.exists():
        from datetime import datetime as _dt
...
async def get_tool_calls(
    ...
    from openinvest.core.llm_telemetry import read_tool_calls
