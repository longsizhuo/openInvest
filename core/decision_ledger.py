...
def _match_trades(
    ...
    explicit = [t for t in trades if t.get("verdict_id") == decision_id]
    if explicit:
        return explicit
    ...
    for t in trades:
        ...
        try:
            ...
            continue
        if d0 <= ts.replace(tzinfo=None) <= d1:
            out.append(t)
    return out
    ...
    for day_dir in sorted(root.iterdir(), reverse=True):
        ...
        for md in sorted(day_dir.glob("*.md")):
            ...
            if not parsed:
                continue
            date = day_dir.name
            symbol = parsed["symbol"] or md.stem
            ...
            if decl is not None:
                ...
                executed = True
            elif parsed["verdict"] in _VERDICT_DIRECTION:
                executed = False  # 有方向的建议、窗口内无成交也无声明 → 视为未执行
